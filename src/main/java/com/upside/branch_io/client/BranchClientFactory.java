...
/**
 * Dropwizard-style Factory that can map from a JSON/YAML configuration file and supports creation
 * of a real client or a mock client.
 */
public class BranchClientFactory {
    ...
    public BranchClient build() {
        ...
        if (MOCK == this.clientType) {
            ...
            LOGGER.warn(
                    "Because Branch.IO apiKey and apiSecret are undefined, defaulting to MockBranchClient.");
            this.clientType = MOCK;
            client = new MockBranchClient();
        } else {
            client =
                    RetrofitBranchClient.create(
                            BranchCredentials.create(this.apiKey, this.apiSecret));
        }

        // Any non-zero retryAttempts value means we should wrap our actual client with a retrying
        // client
