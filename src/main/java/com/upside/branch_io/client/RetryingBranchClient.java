...
/**
 * <p>A wrapper around a branch client that will back off and retry some configurable number of times before really
 * giving up and throwing an exception</p>
 */
public class RetryingBranchClient implements BranchClient {
    ...
    public RetryingBranchClient(BranchClient client, int maxAttempts) {
        Preconditions.checkArgument(maxAttempts <= FIBONACCI.length, "This retrying wrapper only allows backing off '%s' "
            + "times", FIBONACCI.length);
        this.client = client;
        ...
        do {
            try {
                ...
                if (attempt > this.maxAttempts) {
                    throw new RuntimeException("Unable to generate a branch IO link after " + attempt + " attempts, "
                        + "giving up and re-throwing the most recent exception from our client", e);
                }
            }
            attempt++;
        }
